...
export const getTickerPriceData = async (ticker: string, resolution: string): Promise<Result[]> => {
  ...
  const res = await axios.get<TickerResponse>(`https://api.polygon.io/v2/aggs/ticker/${ticker}/range/${range}/${startDate}/${today}?adjusted=true&sort=desc&limit=50000&apiKey=${credentials.API_KEY}`);
